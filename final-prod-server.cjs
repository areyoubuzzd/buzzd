 ...
 * - Handles missing 'logo_url' column
 * - Adds computed 'hasActiveDeals' field for each establishment
 */
...
console.log(`
=================================================
  BUZZD FINAL PRODUCTION SERVER (STARTED: ${new Date().toISOString()})
=================================================
  ...
  try {
    ...
    const formattedRows = establishments.map(row => {
      ...
      return {
        ...
        lat: row.latitude,
        lng: row.longitude,
        neighbourhood: row.city, // Map city to neighbourhood for frontend
        logoUrl: row.imageUrl, // Use image_url as logoUrl (since logo_url doesn't exist)
...
app.get('/api/establishments/:establishmentId', async (req, res) => {
  try {
    ...
    establishment.neighbourhood = establishment.city; // Map city to neighbourhood for frontend
    establishment.logoUrl = establishment.imageUrl; // Use image_url as logoUrl (since logo_url doesn't exist)
...
app.get('/api/establishments/:establishmentId/deals', async (req, res) => {
  try {
    ...
    establishment.neighbourhood = establishment.city; // Map city to neighbourhood for frontend
    establishment.logoUrl = establishment.imageUrl; // Use image_url as logoUrl (since logo_url doesn't exist)
    establishment.hasActiveDeals = dealsWithActiveStatus.some(deal => deal.isActive);
  ...
  try {
    ...
    establishmentRows.forEach(est => {
      establishmentMap[est.id] = {
        ...
        neighbourhood: est.city, // Map city to neighbourhood for frontend
        city: est.city,
        logoUrl: est.image_url, // Use image_url as logoUrl (since logo_url doesn't exist)
    ...
    const nearbyEstablishments = establishments
      .map(est => {
        ...
        return { 
          ...
          lng: est.longitude,
          neighbourhood: est.city, // Map city to neighbourhood for frontend
          logoUrl: est.imageUrl, // Use image_url as logoUrl (since logo_url doesn't exist)
          distance 
        };
      })
