    ...
    <main className="flex-1 bg-gray-50">
      <div className="px-4 sm:px-4 py-2 sm:py-2 pb-24">
        ...
        {filteredDeals.active && filteredDeals.active.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-2 gap-y-3">
            {/* Full view deals (limited by user tier) */}
            {filteredDeals.active.slice(0, fullDealsCount).map((deal: any) => deal && (
              <div className="w-full mb-0 pb-0 px-1" key={deal.id}>
                <DealCard 
                  key={deal.id} 
                  deal={deal} 
            ...
            {filteredDeals.active.slice(fullDealsCount).map((deal: any) => deal && (
              <div className="w-full mb-0 pb-0 px-1" key={`gray-${deal.id}`}>
                <DealCard 
                  key={deal.id} 
                  deal={deal} 
                  userLocation={location}
